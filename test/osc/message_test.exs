defmodule OSC.MessageTest do
  ...
  test "parses a simple integer osc message" do
    ...
    msg =
      <<"/ab", 0, ",iii", 0, 0, 0, 0, 1000::signed-big-size(32), 16::signed-big-size(32),
        17::signed-big-size(32)>>

    assert {"/ab", [{:osc_integer, 1000}, {:osc_integer, 16}, {:osc_integer, 17}]} =
             OSC.Message.parse(msg)
  end
  ...
  test "parses a blob and an int" do
    msg =
      <<"/ab", 0, ",bi", 0, 0x05::signed-big-size(32), 0xBC::signed-big-size(40), 0, 0, 0,
        1000::signed-big-size(32)>>

    assert {"/ab", [{:osc_blob, <<0xBC::signed-big-size(40)>>}, {:osc_integer, 1000}]} =
             OSC.Message.parse(msg)
  end
  ...
  test "parses a color" do
    ...
    assert {"/ab", [{:osc_rgba, [red: 0x01, green: 0x02, blue: 0x03, alpha: 0x04]}]} =
             OSC.Message.parse(msg)

    msg = <<"/ab", 0, ",ri", 0, 0x01020304::size(32), 1000::signed-big-size(32)>>

    assert {"/ab",
            [{:osc_rgba, [red: 0x01, green: 0x02, blue: 0x03, alpha: 0x04]}, {:osc_integer, 1000}]} =
             OSC.Message.parse(msg)
  end
  ...
  test "parses a midi message" do
    ...
    assert {"/ab", [{:osc_midi, [port_id: 0x01, status: 0x02, data1: 0x03, data2: 0x04]}]} =
             OSC.Message.parse(msg)
    ...
    assert {"/ab",
            [
              {:osc_midi, [port_id: 0x01, status: 0x02, data1: 0x03, data2: 0x04]},
              {:osc_integer, 1000}
            ]} = OSC.Message.parse(msg)
  end
  ...
  test "parses a bundle" do
    msg =
      <<"#bundle", 0, 214, 80, 122, 226, 215, 103, 64, 0, 24::size(32)-signed, "/greeting", 0, 0,
        0, ",is", 0, 0, 0, 0, 72, "bob", 0>>
  ...
  test "construct a blob and an int message" do
    assert <<"/ab", 0, ",bi", 0, 0x05::signed-big-size(32), 0xBC::signed-big-size(40), 0, 0, 0,
             1000::signed-big-size(32)>> =
             OSC.Message.construct("/ab", [
               {:osc_blob, <<0xBC::signed-big-size(40)>>},
               {:osc_integer, 1000}
             ])
  end
  ...
  test "construct an rgba message" do
    assert <<"/ab", 0, ",r", 0, 0, 0x01020304::size(32)>> =
             OSC.Message.construct("/ab", [
               {:osc_rgba, [red: 0x01, green: 0x02, blue: 0x03, alpha: 0x04]}
             ])
  ...
  test "construct a bundle" do
    assert <<"#bundle", 0, 214, 80, 122, 226, 0, 0, 0, 0, 24::size(32)-signed, "/greeting", 0, 0,
             0, ",is", 0, 0, 0, 0, 72, "bob",
             0>> =
    ...
    assert <<"#bundle", 0, 214, 80, 122, 226, 0, 0, 0, 0, 20::size(32)-signed, "/greeting", 0, 0,
             0, ",s", 0, 0, "bob",
             0>> =
             OSC.Message.construct(:osc_bundle, {:osc_timetag, {{2013, 12, 9}, {17, 22, 42}}}, [
